...
trait UploadFiles {
    ...
    public function uploadAudios($request, $logBook, $folder = 'logbook/audios/') {
        ...
        if ($request->audios) {
            $getAudios = $this->createFilesFromBase64($request, $request->audios, $folder);
    ...
    function createFilesFromBase64($request, $files, $folder) {
        ...
        foreach ($files as $key => $file_data) {
            ...
            $f = finfo_open();
            $mime_type = finfo_buffer($f, base64_decode($file), FILEINFO_MIME_TYPE);
            $type = $this->mime2ext($mime_type);
            if ($type and $file != "") {
                Storage::disk('public')->put($file_name . "." . $type, base64_decode($file));
